...
def measure_error(r, h_list, t, exact_U, type, rand_states=[], ob=None, pf_ord=2, coeffs=[], use_jax=False, verbose=False, return_error_list=False): 
    ...
    elif type == 'lightcone_bound_table':
        n = coeffs[1]
        def temp_bound(r, t, c1, c2):
            return 2 * (c1 * (t/r)**3 / 12 + c2 * (t/r)**3 / 24) 
        if coeffs[0] == 'singl':
            result = [temp_bound(r, t, coeffs[2][j-1][0], coeffs[2][j-1][1]) for j in range(1, r+1)]
        elif coeffs[0] == 'multi_z':
            result = [temp_bound(r, t, sum(coeffs[2][j-1][:, 0]), sum(coeffs[2][j-1][:, 1]))/n for j in range(1, r+1)]
        elif coeffs[0] == 'multi_zz':
            result = [temp_bound(r, t, sum(coeffs[2][j-1][:, 0]), sum(coeffs[2][j-1][:, 1]))/(n-1) for j in range(1, r+1)]
        else:
            raise ValueError(f'coeffs[0]={coeffs[0]} is not defined!')
        return sum(result)
